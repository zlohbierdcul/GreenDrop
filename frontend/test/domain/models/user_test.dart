...
void main() {
  group('User Model Tests', () {
    test('Should parse JSON correctly into User object', () {
      final jsonMap = {
        'documentId': 'user123',        // -> user.id
        'id': 'someUserId',            // -> user.userId
        'user_detail': {
          'documentId': 'uDetail123',   // -> user.userDetailId
          'username': 'john_doe',
          'first_name': 'John',
          'last_name': 'Doe',
      ...
      expect(user.userId, 'someUserId');
      expect(user.userDetailId, 'uDetail123');
    ...
    test('Should convert User object to JSON correctly', () {
      ...
      expect((json['addresses'] as List).first, 'address123');
    });
    ...
    test('Should parse a list of Users from JSON string', () {
      ...
      expect(users[0].userId, 'uid1');
      expect(users[0].userName, 'john_doe');
      expect(users[0].addresses.first.city, 'Berlin');

      // user2
      expect(users[1].id, 'user456');
      expect(users[1].userId, 'uid2');
      expect(users[1].firstName, 'Jane');
      expect(users[1].greenDrops, 200);
